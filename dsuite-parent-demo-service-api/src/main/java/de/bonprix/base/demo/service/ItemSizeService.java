...
public interface ItemSizeService {
    ...
    @GET
    @Path("/")
    List<ItemSize> findAll(@BeanParam @Valid Paged pageable);
    ...
    @POST
    @Path("/filtered")
    List<ItemSize> findByFilter(ComplexFilter complexFilter);
    ...
    @GET
    @Path("/{id : \\d+}")
    ItemSize findById(@PathParam("id") @NotNull Long id);
    ...
    @PUT
    @Path("/")
    ItemSize saveItemSize(@NotNull ItemSize itemSize);
