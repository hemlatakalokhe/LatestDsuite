...
public interface LoginService {
    ...
    @GET
    @Path("/")
    List<Login> findAll(@BeanParam @Valid Paged pageable);
    ...
    @POST
    @Path("/filtered")
    List<Login> findByFilter(ComplexFilter complexFilter);
    ...
    @GET
    @Path("/{id : \\d+}")
    Login findById(@PathParam("id") @NotNull Long id);
    ...
    @PUT
    @Path("/")
    Login saveLogin(@NotNull Login login);

    @GET
    @Path("/validateUser")
    boolean validateUser(@BeanParam String username, @BeanParam String password);
