...
public interface PlanperiodService {
    ...
    @GET
    @Path("/")
    List<Planperiod> findAll(@BeanParam @Valid Paged pageable);
    ...
    @POST
    @Path("/filtered")
    List<Planperiod> findByFilter(ComplexFilter complexFilter);
    ...
    @GET
    @Path("/{id : \\d+}")
    Planperiod findById(@PathParam("id") @NotNull Long id);
    ...
    @PUT
    @Path("/")
    Planperiod savePlanperiod(@NotNull Planperiod planperiod);
