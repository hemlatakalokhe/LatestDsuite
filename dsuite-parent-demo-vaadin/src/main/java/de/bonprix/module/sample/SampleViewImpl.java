...
@SpringView(name = SampleViewImpl.VIEW_NAME, ui = { VaadinUI.class }, isDefault = false, order = 30)
public class SampleViewImpl extends AbstractMvpView<SamplePresenter> implements SampleView {
    ...
    protected void initializeUI() {
        ...
        this.countryComboBox.addValueChangeListener(event->this.notificationProvider.showInfoMessageBox("U selected"+ this.countryComboBox.getSelectedItem().toString()));

        setCompositionRoot(FluentUI.vertical()
                           .add(FluentUI.button().caption("Show Info Message")
                                .onClick(event->this.notificationProvider.showInfoNotification(SampleViewImpl.CAPTION, SampleViewImpl.MESSAGE)).get())
                           .add(FluentUI.button().caption("Show Info messasge (Only Caption) ")
                                .onClick(event->this.notificationProvider.showInfoNotification(SampleViewImpl.CAPTION)).get())
                           .add(FluentUI.button().caption("Show Info Message Custom")
                                .onClick(event->this.notificationProvider.showInfoNotification(notificationFieldGroupBean.getCaption(), notificationFieldGroupBean.getMessage(), notificationFieldGroupBean.getDelay())).get())
                           .add(FluentUI.button().caption("Show Error Message")
                                .onClick(event->this.notificationProvider.showErrorNotification(SampleViewImpl.CAPTION, SampleViewImpl.MESSAGE)).get())
                           .add(FluentUI.button().caption("Show Error Message Custom")
                                .onClick(event->this.notificationProvider.showErrorNotification(notificationFieldGroupBean.getCaption(), notificationFieldGroupBean.getMessage(), notificationFieldGroupBean.getDelay())).get())
                           .add(FluentUI.button().caption("Show Warning Message")
                                .onClick(event->this.notificationProvider.showWarningNotification(SampleViewImpl.CAPTION, SampleViewImpl.MESSAGE)).get())
                           .add(FluentUI.button().caption("Show Warning Message Custom")
                                .onClick(event->this.notificationProvider.showWarningNotification(notificationFieldGroupBean.getCaption(), notificationFieldGroupBean.getMessage(), notificationFieldGroupBean.getDelay())).get())
                           .add(FluentUI.button().caption("Image Browser")
                                .onClick(event->openImageBrowser(getImageUrls(), ZoomType.MAGNIFIER)).get())
                           .add(FluentUI.button().caption("Image Browser Slider")
                                .onClick(event->openImageBrowser(getImageUrls(), ZoomType.SLIDERZOOM)).get())
